...
### BL-59 — Geri dönüşte efekt oynuyordu; kabuk elipse dönüşüp takılıyordu (2026-09-09)
- Gözlem (kullanıcı): *"**Geri gelince ana ekran geliyor sonra saçma bir efekt geliyor**; ona gerek yok, zaten ana ekran gelmiş."* ve *"Efekt çok saçma la, **yuvarlak köşeli saçma oval bir şey** geliyor, **takılıyor gibi**, bunu en çok Kutu'ya basınca hissediyorum."* Üç ayrı kusur, üçü de gerçekti.
- **1) Geri dönüş ileri gidiş sayılıyordu.** Uygulama içindeki "← Bugün" / "← İçerik" düğmeleri `ctx.navigate(...)` çağırıyor; sistem bunu yeni bir gidiş sayıp tam açılma efektini (kabuk + perde + kademeli varış) oynatıyordu. Ekran zaten oradan gelinmişti, yeniden "açmak" saçmaydı. Yama: `navigate(screen, { back: true })` — on bir "←" düğmesinin hepsi bu bayrağı geçiriyor; geri dönüş `pop` sayılıyor, kabuk **hiç kurulmuyor** ve `pop` yalnız 140 ms'lik bir belirmeye indi (sert takas görüntüsü olmasın diye tamamen kaldırılmadı).
- **2) "Oval bir şey" — gerçekten elipsti.** BL-57'de kabuk ekran boyunda bir kutuya çevrilip `transform: scale()` ile büyütülüyordu; kapsül düğmenin `border-radius: 999px` değeri **tam ekran bir kutuda %50'ye kırpılıyor**, yani dev bir elips oluyordu. Kutu ne kadar küçültülürse küçültülsün şekil elips kalıyordu (ölçek yarıçapı da ölçekler). Yama: kabuk yeniden **gerçek kutu geometrisiyle** büyür (`left/top/width/height`), yarıçap `min(hesaplanan, genişlik/2, yükseklik/2)` ile başlar ve 0'a iner — büyüyen kutuda yarıçap ve kenar **dürüst** kalır.
- **3) "Takılıyor gibi" — kabukta `backdrop-filter` vardı.** Kabuğun kutusu her karede değiştiği için bulanıklık her karede yeniden hesaplanıyordu; telefonda takılmanın sebebi buydu. Yama: kabuktan `backdrop-filter` kaldırıldı, bulanıklığı **alttaki perde** verir (o sabit, yalnız opaklığı değişir). Kabuk yarı saydam olduğu için içinden bulanık zemin görünür: his aynı, maliyet düşük.
- **Çapraz süpürme korundu ve düzeldi.** Ölçek yerine ara kare kullanılıyor: dokunuşa **yakın** kenarlar yerinde kalırken **uzak** kenarlar önce ekranın sonuna gider. Gerçek tarayıcıda ölçüldü (390×800): sağ taraftan dokunuşta ara kare `left 0 · width 281` (sol-üste fırlar), sol taraftan dokunuşta `left 20.8 · width 369` (sağ-üste fırlar). Kapsül düğmede başlangıç yarıçapı 28.6 px — kutunun tam yarısı, yani doğru kapsül.
- Testler: kabuk dokunulan öğenin gerçek kutusundan başlar ve yarıçapı kutunun yarısını aşmaz; çapraz süpürme dokunuşun tarafına göre farklı ara kare üretir; tam genişlikteki alçak öğede kabuk ekranın %10'undan küçük başlar; "← Bugün" kabuk kurmaz ve `screen-pop` ile gelir. Toplam 356 yeşil.
- Durum: **KAPANDI**. Telefon kontrolü: Y-36.
